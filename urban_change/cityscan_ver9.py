    ...
    for file in os.listdir(admin_folder):
        if file.endswith(".shp"):  
            ...
            with rasterio.open(input_raster) as src:
                ...
                output_4326_raster_clipped = output_folder + prepend_file_text + "_4326_clipped.tif"
                ...
                with rasterio.open(output_utm_raster_clipped) as src:
                    ...
                    for year in range(1985, 2016):
                        ...
                        year_dict[year] = np.count_nonzero(array == year) * pixelSizeX * pixelSizeX/ 1000000
                ...
                output_4326_raster_clipped_reclass = output_folder + prepend_file_text + "_WSF_4326_reclass" + "_%s.tif" % file[:-4]
                ...
                fileList = glob.glob(output_4326_raster_clipped[:-3]+'*', recursive = True)
                for filePath in fileList:
                  try:
                      os.remove(filePath)
                  except OSError as e:
                      print("Error while deleting file")
                      print("Failed with:", e.strerror) # look what it says
                fileList = glob.glob(output_utm_raster_clipped[:-3]+'*', recursive = True)
                for filePath in fileList:
                  try:
                      os.remove(filePath)
                  except OSError as e:
